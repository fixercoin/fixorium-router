...
const Dashboard: React.FC<DashboardProps> = ({ walletAddress = '' }) => {
    ...
    const testMaxEndpoint = async () => {
        ...
        if (!apiKey) {
            setMaxResponse('Error: No MAX API key found. Please register first.');
            setMaxLoading(false);
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-dark">
            {/* Fixed Header - ALL TEXT ON LEFT, PROFILE ON RIGHT */}
            <header className="fixed top-0 left-0 right-0 bg-darker/95 backdrop-blur-md border-b border-border z-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-12">
                        {/* LEFT SIDE - All navigation text */}
                        <div className="flex items-center gap-6">
                            <div className="text-[10px] font-semibold text-white uppercase tracking-wider">
                                DEFI PLATFORM
                            </div>
                            <nav className="hidden md:flex items-center gap-5">
                                <button onClick={() => window.location.href = '/'} className="text-[10px] text-white hover:text-primary transition uppercase tracking-wider">
                                    HOME
                                </button>
                                <a href="https://exchange.fixorium.com.pk" target="_blank" rel="noopener noreferrer" className="text-[10px] text-white hover:text-primary transition uppercase tracking-wider">
                                    EXCHANGE
                                </a>
                                <a href="https://wallet.fixorium.com.pk" target="_blank" rel="noopener noreferrer" className="text-[10px] text-white hover:text-primary transition uppercase tracking-wider">
                                    WALLET
                                </a>
                                <a href="https://fixorium.com.pk/team" target="_blank" rel="noopener noreferrer" className="text-[10px] text-white hover:text-primary transition uppercase tracking-wider">
                                    TEAM
                                </a>
                            </nav>
                        </div>
                        ...
                        <div className="relative">
                            <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 text-white hover:text-primary transition p-1">
                                        ...
                                        {isRegistered && (
                                            <>
                                                <div className="px-4 py-3 border-b border-border">
                                                    ...
                                                    <div className="text-[10px] text-white break-all">{registeredEmail}</div>
                                                </div>
                                                <div className="px-4 py-3 border-b border-border">
                                                ...
                                                <div className="px-4 py-3 border-t border-border mt-2">
                                                    <button onClick={handleLogout} className="w-full text-left px-2 py-1 text-[10px] text-red-400 hover:bg-red-500/10 transition uppercase tracking-wider">LOGOUT</button>
                                                </div>
                                            </>
                                        )}
                                    </div>
            ...
            {/* Custom Marquee */}
            <div className="fixed top-12 left-0 right-0 bg-primary/10 border-y border-primary/20 overflow-hidden whitespace-nowrap py-2 z-40">
                <div className="inline-block animate-marquee whitespace-nowrap">
                    <span className="mx-4 inline-flex items-center gap-2">
                        ...
                        <span className="text-white text-[10px] uppercase tracking-wider">FIXORIUM EXCHANGE — MULTICHAIN DEX AGGREGATOR</span>
                    </span>
                    <span className="mx-4 inline-flex items-center gap-2">
                        <span className="text-yellow-400 text-[10px] uppercase tracking-wider font-semibold">WALLET</span>
                        <span className="text-white text-[10px] uppercase tracking-wider">FIXORIUM WALLET — MULTICHAIN WALLET</span>
                    </span>
                    <span className="mx-4 inline-flex items-center gap-2">
                        <span className="text-yellow-400 text-[10px] uppercase tracking-wider font-semibold">API</span>
                        <span className="text-white text-[10px] uppercase tracking-wider">7 ENDPOINTS: QUOTE • SWAP • POOLS • TOKEN PRICE • ACCOUNT • LIMIT ORDER • DCA</span>
                    </span>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="lg:w-[30%] flex">
                            <div className="w-full border border-gray-700 rounded-xl overflow-hidden flex flex-col h-full">
                                <div className="p-4 border-b border-gray-700">
                                    <h1 className="text-[10px] font-bold text-white uppercase tracking-wider">API DASHBOARD</h1>
                                    <p className="text-[10px] text-gray-400 mt-1">Welcome back, <span className="text-primary">{registeredEmail}</span></p>
                                </div>

                                <div className="p-4 border-b border-gray-700">
                                    <h3 className="text-[10px] font-bold text-white uppercase tracking-wider mb-3">API USAGE</h3>
                                    <div className="space-y-3">
                                        <div>
                                            <div className="flex justify-between text-[10px] mb-1">
                                                <span className="text-white">MAX API (Solana)</span>
                                                <span className="text-white">{apiUsage.maxCalls} / {apiUsage.maxLimit}</span>
                                            </div>
                                        ...
                                        <div>
                                            <div className="flex justify-between text-[10px] mb-1">
                                                <span className="text-white">MintMe API (EVM)</span>
                                                <span className="text-white">{apiUsage.mintMeCalls} / {apiUsage.mintMeLimit}</span>
                                            </div>
                                            <div className="w-full bg-darker rounded-full h-1.5">
                                ...
                                <div className="p-4 border-b border-gray-700">
                                    <div className="flex gap-6">
                                        <button
                                            ...
                                            className={`text-[10px] font-semibold uppercase tracking-wider transition-all pb-2 ${activeTab === 'max' ? 'text-primary border-b-2 border-primary' : 'text-white hover:text-gray-300'}`}
                                        >
                                            MAX API
                                        </button>
                                        <button
                                            onClick={() => setActiveTab('mintme')}
                                            className={`text-[10px] font-semibold uppercase tracking-wider transition-all pb-2 ${activeTab === 'mintme' ? 'text-primary border-b-2 border-primary' : 'text-white hover:text-gray-300'}`}
                                        >
                                            MINTME API
                                        </button>
                                    </div>
                                ...
                                <div className="p-4 flex-1">
                                    <div className="text-[10px] text-white uppercase mb-2">
                            ...
                            {activeTab === 'max' ? (
                                <div className="w-full border border-gray-700 rounded-xl overflow-hidden flex flex-col h-full">
                                    <div className="p-4 border-b border-gray-700">
                                        <div className="flex items-center justify-between flex-wrap gap-3">
                                            <div>
                                                <h2 className="text-[10px] font-bold text-white uppercase tracking-wider">MAX API TESTER</h2>
                                                <p className="text-[10px] text-gray-400 mt-1">Solana DEX Aggregator - 0.01% Fee</p>
                                            </div>
                                            <div className="flex flex-wrap gap-2">
                                                <select
                                                    value={maxEndpoint}
                                                    onChange={(e) => setMaxEndpoint(e.target.value)}
                                                    className="px-3 py-1.5 text-[10px] font-semibold rounded-lg border border-gray-700 bg-darker text-white focus:border-primary outline-none"
                                                >
                                                    {maxEndpoints.map(ep => (
                                    ...
                                    <div className="p-4 flex-1">
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
                                            <div className="flex flex-col">
                                                <label className="block text-[10px] text-white uppercase mb-2 font-semibold">REQUEST PARAMETERS</label>
                                                <textarea 
                                                    value={maxParams} 
                                                    onChange={(e) => setMaxParams(e.target.value)} 
                                                    className="flex-1 w-full p-3 bg-darker border border-gray-700 rounded-lg text-white text-[10px] font-mono focus:border-primary outline-none resize-none min-h-[350px]" 
                                                    placeholder="Enter JSON parameters..."
                                                />
                                                <div className="flex items-center justify-between mt-3">
                                                    <div className="text-[10px] text-gray-400">Program ID: {MAX_PROGRAM_ID.slice(0, 16)}...</div>
                                                    <button 
                                                        onClick={testMaxEndpoint} 
                                                        disabled={maxLoading || !apiKey} 
                                                        className="px-5 py-2 bg-primary text-black text-[10px] font-bold rounded-lg hover:bg-[#e8d58a] transition disabled:opacity-50"
                                                    >
                                            ...
                                            <div className="flex flex-col">
                                                <label className="block text-[10px] text-white uppercase mb-2 font-semibold">RESPONSE</label>
                                ...
                                <div className="w-full border border-gray-700 rounded-xl overflow-hidden flex flex-col h-full">
                                    <div className="p-4 border-b border-gray-700">
                                        <div className="flex items-center justify-between flex-wrap gap-3">
                                            <div>
                                                <h2 className="text-[10px] font-bold text-white uppercase tracking-wider">MINTME API TESTER</h2>
                                                <p className="text-[10px] text-gray-400 mt-1">EVM DEX Aggregator - 0.01% Fee</p>
                                            </div>
                                            <div className="flex flex-wrap gap-2">
                                                <select
                                    ...
                                    <div className="p-4 flex-1">
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
                                            <div className="flex flex-col">
                                                ...
                                                />
                                                <div className="flex items-center justify-between mt-3">
                                                    <div className="text-[10px] text-gray-400">Contract: {MINTME_CONTRACT.slice(0, 16)}...</div>
                                                    <button 
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-darker/95 backdrop-blur-md border-t border-border z-50 md:hidden">
                <div className="flex items-center justify-around py-2">
                    <button onClick={() => window.location.href = '/'} className="flex flex-col items-center gap-1 text-white hover:text-primary transition">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                        <span className="text-[8px] uppercase">HOME</span>
                    </button>
                    <a href="https://exchange.fixorium.com.pk" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 text-white hover:text-primary transition">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m3 4H4m0 0l4 4m-4-4l4-4" /></svg>
                        <span className="text-[8px] uppercase">EXCHANGE</span>
                    </a>
                    <a href="https://wallet.fixorium.com.pk" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 text-white hover:text-primary transition">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M6 14h12M9 18h6M12 6v12" /></svg>
                        <span className="text-[8px] uppercase">WALLET</span>
                    </a>
                    <a href="https://fixorium.com.pk/team" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 text-white hover:text-primary transition">
